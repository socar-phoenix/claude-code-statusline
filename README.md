...
# 2. settings.json 및 pass-through 정리
node -e "
  ...
  if (cmd.includes('statusline.js')) {
    ...
    console.log('statusLine이 다른 도구를 가리키고 있어 settings.json은 변경하지 않았습니다');
  } else {
    console.log('statusLine 설정이 없습니다');
  }
  // socar-board pass-through 파일 정리
  const pt = os.homedir() + '/.config/socar-board/original_statusline_cmd';
  try {
    const c = fs.readFileSync(pt, 'utf8');
    if (c.includes('statusline.js')) { fs.unlinkSync(pt); console.log('pass-through 설정 제거 완료'); }
  } catch {}
"
```
